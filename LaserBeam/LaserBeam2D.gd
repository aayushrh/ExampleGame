...
func _physics_process(delta: float) -> void:
	...
	cast_beam()
	#finding the direction to cast the beam in
...
func cast_beam() -> void:
	...
	#find anythign the laser should be hitting
	collision_particles.emitting = is_colliding()
	#emit the collision particles if hitting something
	...
	if is_colliding():
		...
		#put the collision particles to where the laser is hitting
	else:
		$Hitbox/CollisionShape2D.disabled = true
		#if its not collidion with anything 
